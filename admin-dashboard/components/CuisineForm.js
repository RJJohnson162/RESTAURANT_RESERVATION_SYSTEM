...
export default function CuisineForm({
  ...
  const [cuisineProperties, setCuisineProperties] = useState(
    assignedProperties || {}
  );
  ...
  const saveCuisine = async (ev) => {
    ...
    if (_id) {
      //update
      await axios.put("/api/cuisine", { ...data, _id });
    } else {
      //create
      await axios.post("/api/cuisine", data);
    }
    setGotoCuisine(true);
  ...
  function cancel() {
    router.push("/cuisine"); // Redirect to the "Cuisine" page
  }

  // Check if we are in edit mode based on the current path
  const isEditMode = router.pathname.includes("/Cuisine/edit");
      ...
      {propertiesToFill.length > 0 &&
        propertiesToFill.map((p) => (
          <div key={p.name} className="">
            ...
            <div>
              <select
                value={cuisineProperties[p.name]}
                onChange={(ev) => {
