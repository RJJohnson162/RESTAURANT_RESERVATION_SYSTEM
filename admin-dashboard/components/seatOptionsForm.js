...
export default function SeatOptionsForm({
  _id,
  title: existingTitle,
  ...
  useEffect(() => {
    axios.get("/api/seatCategories").then((result) => {
  ...
  const saveCuisine = async (ev) => {
    ...
    if (_id) {
      //update
      await axios.put("/api/seatOptions", { ...data, _id });
    } else {
      //create
      await axios.post("/api/seatOptions", data);
    }
    setGotoCuisine(true);
  ...
  function setCuisineProp(propName, value) {
    setSeatOptionProperties((prev) => {
      const newCuisineProps = { ...prev };
      newCuisineProps[propName] = value;
      return newCuisineProps;
    });
  }
  ...
  function cancel() {
    router.push("/seatOptions"); // Redirect to the "seatOption" page
  }

  // Check if we are in edit mode based on the current path
  const isEditMode = router.pathname.includes("/seatOption/edit");

  return (
    <form onSubmit={saveCuisine}>
      {formChanged && ( // Display warning message when changes have been made
        <div className="bg-primary border flex-col w-3/5 text-white rounded-lg mb-4 flex items-center justify-center p-2">
      ...
      <label>Seating Arrangement name</label>
      <input
        type="text"
        placeholder="Seating Arrangement name"
        value={title}
        onChange={(ev) => {
      ...
      >
        ...
        {categories.length > 0 &&
          categories.map((c) => (
            <option
              className="border-b border-white text-white bg-primary rounded-md hover:bg-white hover:text-black"
              key={c._id}
              value={c._id}
            >
              {c.name}
      ...
      {propertiesToFill.length > 0 &&
        propertiesToFill.map((p) => (
          <div key={p.name} className="">
            ...
            <div>
              <select
                value={seatOptionsProperties[p.name]}
                onChange={(ev) => {
                  setCuisineProp(p.name, ev.target.value);
                  handleChange(); // Track form changes
                }}
              >
                {p.values.map((v) => (
        ...
        >
          {!!images?.length &&
            images.map((link, index) => (
              <div
                key={link}
                className="h-24 bg-primary p-4 shadow-sm flex flex-col gap-1 items-center border border-gray-200 mb-4 mt-2 rounded-md"
              >
                <img src={link} alt="" className="rounded-lg" />
                <button
                  onClick={() => deleteImage(index)}
                  className="text-red-100 border bg-primary rounded-md p-1 hover"
                >
                  Delete
                </button>
              </div>
